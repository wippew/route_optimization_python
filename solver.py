...
def solveAndDraw(df, duration, task_count, vehicle_count, vehicle_capacity):
    ...
    # for k in range(vehicle_count):
    #     problem += pulp.lpSum(x[0][j][k] for j in range(1, task_count)) == 1
    #     problem += pulp.lpSum(x[i][0][k] for i in range(1, task_count)) == 1
    ...
    for k in range(vehicle_count):
        problem += pulp.lpSum(
            ...
            range(1, task_count)) <= vehicle_capacity
    ...
    for i in range(task_count):
        if i == 0:
            ...
            plt.text(df.longitude[i], df.latitude[i], "depot_2", fontsize=12)
        else:
